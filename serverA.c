...
#define MYPORT "21841" // the port users will be connecting to
...
int main(void)
{
	...
	/* Operation code or Function code received by Backend Server A from aws server*/
		...
		/* Number of elements received by Backend Server A from aws server*/
		...
		/* Elements received by Backend Server A from aws server*/
	...
	/* If operation code is sum then find the sum of elements and send to aws	*/
		...
		sum =0;
			...
			printf("The Server A has successfully finished the reduction SUM: %d\n",sum);
	...
	/* If operation code is max then find the max of elements and send to aws*/
		...
		if(opcode==2)
		{
			...
			maximum = &max;
			printf("The Server A has successfully finished the reduction MAX: %d\n",max);
		...
		if(opcode==3)
		{
			...
			minimum = &min;
			printf("The Server A has successfully finished the reduction MIN: %d\n",min);
		...
		sumofsquares = 0;
			...
			printf("The Server A has successfully finished the reduction SOS: %d\n",sumofsquares);
			sos = &sumofsquares;
