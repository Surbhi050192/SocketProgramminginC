...
#include <arpa/inet.h>
#include <netdb.h>
#define MYPORT "23841" // the port users will be connecting to
...
int main(void)
{
	...
	/* Operation code or Function code received by Backend Server C from aws server*/
		...
		/* Number of elements received by Backend Server C from aws server*/
		...
		/* Elements received by Backend Server C from aws server*/
		...
		sum =0;
			...
			printf("The Server C has successfully finished the reduction SUM: %d\n",sum);
		...
		if(opcode==2)
		{
			...
			maximum = &max;
			printf("The Server C has successfully finished the reduction MAX: %d\n",max);
		...
		if(opcode==3)
		{
			...
			minimum = &min;
			printf("The Server C has successfully finished the reduction MIN: %d\n",min);
		...
		sumofsquares = 0;
			...
			printf("The Server C has successfully finished the reduction SOS: %d\n",sumofsquares);
			sos = &sumofsquares;
